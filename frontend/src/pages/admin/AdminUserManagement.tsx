...
const AdminUserManagement = () => {
    ...
    const [logs, setLogs] = useState<TokenLog[]>([]); // ✅ 하단 테이블에서 사용하도록 수정
    ...
    const fetchAllUsers = useCallback(async () => {
        try {
            ...
            console.error("전체 유저 로딩 실패:", err);
        }
    }, [getAuthHeader]);

    // ✅ useEffect 내의 비활성 상태 및 비동기 함수 호출 수정
    useEffect(() => {
        if (isAuthenticated && role === 'ADMIN' && accessToken) {
            // async 함수를 즉시 실행 함수(IIFE)로 감싸서 await 처리
            (async () => {
                await fetchLogs();
                await fetchAllUsers();
            })();
        }
    }, [isAuthenticated, role, accessToken, fetchLogs, fetchAllUsers]);
    ...
    return (
        <div style={s.container}>
            ...
            <div style={s.logSection}>
                <h3 style={s.sectionSubTitle}>📋 전체 유저 리스트</h3>
